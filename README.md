# proyecto1-spa-beauty
<!doctype html>
<html class="no-js" lang="">

<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>CSS Proyecto 1 - Spa & Beauty</title>
  <link rel="stylesheet" href="css/style.css">
  <meta name="description" content="">

  <meta property="og:title" content="">
  <meta property="og:type" content="">
  <meta property="og:url" content="">
  <meta property="og:image" content="">

  <link rel="icon" href="/favicon.ico" sizes="any">
  <link rel="icon" href="/icon.svg" type="image/svg+xml">
  <link rel="apple-touch-icon" href="icon.png">

  <link rel="manifest" href="site.webmanifest">
  <meta name="theme-color" content="#fafafa">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@200;300;400;500;700;800&display=swap"
    rel="stylesheet">
</head>

<body>


  <div class="page">

    <header>
      <figure>
        <img src="img/logo-glowqueen.png" alt="Logotipo" srcset="img/logo-glowqueen@2x.png 2x">
        <figcaption>Logotipo de logo-glowqueen</figcaption>
      </figure>

      <nav>
        <ul>
          <li><a href="">Home</a></li>
          <li><a href="">About</a></li>
          <li><a href="">Services</a></li>
          <li><a href="">Facilities</a></li>
          <li class="dropdown">
          <a href="#">More <span class="arrow">▼</span></a>
          <ul class="dropdown-menu">
          
        </ul>
      </nav>

      <div class="login"> 
        <ul>
          <li><a href="#"><img src="img/search.svg" alt="Buscar"></a></li>
          <li><a href="#"><img src="img/shopping-bag.svg" alt="Shopping"></a></li>
          <li><a href="#" class="btn-login">Sign in</a></li>
          <li><a href="#" class="btn-login">Sign up</a></li>
        </ul>
      </div>

    </header>

    <section class="hero">
        <div class="hero__content">
          <h1> Bring back the beauty and glow of your skin</h1>
          <p>We provide you various treatments from head to toe using the best product, advanced technology, and affordable price</p>
          
          <div class="hero__buttons">
            <a href="#" class="btn-primary">Book Now</a>

            <a href="#" class="btn-play">
              <img src="img/icon.svg" alt="Play Icon">
              <span>Watch Video</span>
            </a>
          </div>
       </div>
    </section>

    <section class="categories">
      <article>
        <img src="img/face-icon.svg" alt="">
        <h3>15+</h3>
        <p>Special<br>Treatment</p>
      </article>
      <article>
        <img src="img/flor-icon.svg" alt="">
        <h3>25+</h3>
        <p>Professional<br>Therapist</p>
      </article>
      <article>
        <img src="img/fruta-icon.svg" alt="">
        <h3>100%</h3>
        <p>Herbal<br>Cream Formula</p>
      </article>
    </section>

   <section class="testimonios">
   
     <img src="img/beauty-woman.svg" alt="Foto chica" class="foto-chica" />
      <article class="testimonio1">
        <img src="img/comillas.svg" alt="comillas" class="comillas" />
        <p>I have a regular schedule to take treatment from GlowQueen, they gave me the best service and treatment ever to maintain the beauty and health of my skin.</p>
        <strong>Sophia Princeton</strong>
      </article>

      <article class="testimonio2" >
        <img src="img/comillas.svg" alt="comillas" class="comillas" />
        <p>For the past few years, I’ve had a hard time finding the right place for skin care to deal with my complicated skin condition, thank God I found GlowQueen to treat my skin.</p>
        <strong>Tamara Jules</strong>
      </article>

   </section>

  </div>

</body>

</html>

